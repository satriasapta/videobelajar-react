    ...
    return (
        <>
            <nav className='w-full flex items-center h-[4.5rem] justify-between shadow border-t border-b border-zinc-100 bg-white fixed z-20'>
                <div className="mx-6 sm:mx-16 flex justify-between items-center w-full">
                    <img src={logo} alt="" />
                    ...
                    {
                        open &&
                        <div ref={menuRef} className="hidden sm:flex bg-white p-4 w-36 shadow-lg absolute z-10 right-14 top-20">
                            <ul className='w-full'>
                                {
                                    menu.map((item, index) => (
